...
void SHT21::crcCalc(uint8_t x) {
    crc8 = pgm_read_byte(crcTab + (x ^ crc8));
}

static void dummyCRC(uint8_t) {}

// static variables, i.e. CRC can only be enabled for all SHT21 devices at once
// the code below avoids linking in any CRC code if enableCRC() is never called

uint8_t SHT21::crc8 = 0;
void (*SHT21::crcFun)(uint8_t) = &dummyCRC;

void SHT21::enableCRC() {
    SHT21::crcFun = crcCalc;
}
...
void SHT21::softReset() const {
    connReset();
    writeByte(RESET);
    delay(11);
}

uint8_t SHT21::readStatus() const {
    start();
...
void SHT21::writeStatus(uint8_t value) const {
    start();
...
void SHT21::calculate(float& rh_true, float& t_C) const {
	...
	t_C = -46.85 + 175.72 / 16384.0 * meas[TEMP];
	
	rh_true = -6.0 + 125.0 / 4096.0 *  meas[HUMI];
	
    if (rh_true > 99) rh_true = 100;
    if (rh_true < 0.1) rh_true = 0.1;
} 

float SHT21::dewpoint(float h, float t) {
