...
class MeasurementScheduler(object):
        ...
        while True:
            ...
            if message is not None:
                ...
                if message['channel'] == b'log_measurements':
                    ...
                    m = self.db_session.query(Parameter).filter(Parameter.id == data['caller_id']).first()
    ...
    def recalculate_schedule(self):
        ...
        controllers = self.db_session.query(PeripheryController).filter(PeripheryController.active is True).all()
        ...
        if len(sensor_ids) > 0:
            ...
            for m in measurements:
                ...
                logging.info(
                    'every ' + str(m.interval) + ' seconds measure: ' + m.parameter.name + ' of ' + m.location.name)
        else:
