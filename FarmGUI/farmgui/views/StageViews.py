...
class StageViews(object):
    ...
    def stage_new(self):
        add_form = Form(StageSchema(), formid='addForm', buttons=('Save',), use_ajax=True)
        ...
        if 'Save' in self.request.POST:
            ...
            try:
                ...
                return Response('database error (query PlantSettings for id)', content_type='text/plain',
                                status_int=500)
            try:
                values = add_form.validate(controls)
                new_stage = Stage(plant_setting, values['Number'], values['Duration'], values['Name'],
                                  values['Description'])
                DBSession.add(new_stage)
                return HTTPFound(location=self.request.route_url('plant_settings_view', _id=plant_setting.id))
